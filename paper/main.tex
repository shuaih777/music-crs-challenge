...
Conversational music recommendation systems aim to provide personalized music suggestions through natural language interactions, enabling users to iteratively refine their preferences across multiple dialogue turns. The RecSys Challenge 2026~\cite{recsys2026} introduces a conversational music recommendation task over 47,071 tracks, where systems must output a ranked list of 20 tracks and a natural language response at each turn.
...
We form a candidate pool by taking the union of all 13 legs' top-100 outputs (~200-400 unique candidates per turn). A LightGBM LambdaRank model is trained to re-rank candidates with the following features per candidate:
...
The TalkPlayData-Challenge dataset~\cite{talkplay2026} contains 15,199 training and 1,000 development sessions, each with 8 conversation turns. The track catalog has 47,071 items with metadata, tags, and pre-computed embeddings across 6 modalities.
